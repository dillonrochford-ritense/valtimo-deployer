...
class ZakenApiPlugin(
    ...
    fun createZaak(
        ...
        withLoggingContext(
            ...
            val caseDocumentId = UUID.fromString(execution.businessKey)
            val extension: Verlenging? = if (extensionReason != null || extensionDuration != null) {
                Verlenging(reden = extensionReason, duur = extensionDuration)
            } else null
            val suspension: Opschorting? = if (suspensionIndication != null || suspensionReason != null) {
                Opschorting(indicatie = suspensionIndication?.toBoolean() ?: false, reden = suspensionReason ?: "")
            } else null
            val processObject: Processobject? = if (
                processObjectDateAttribute != null || processObjectIdentification != null ||
    ...
    fun createVestigingZaakRol(
        ...
        withLoggingContext(
            ...
            logger.debug {
                "Creating vestiging zaakrol with roltype URL '$roltypeUrl' for " +
                    "document with id '${execution.businessKey}'"
            }
    ...
    private fun addZaakToRelevanteAndereZaken(zaakUrl: URI, teRelaterenZaakUri: URI, aardRelatie: String) {
        ...
        if (currentRelevanteAndereZaken.none { relevanteZaak -> relevanteZaak.url == currentZaak.url }) {
            currentRelevanteAndereZaken.add(
                RelevanteZaak(
                    teRelaterenZaakUri,
                    AardRelatie.entries.find { it.key == aardRelatie }!!
                )
            )
    ...
    private fun resolveProperties(
        zaakObjectRequest: ZaakObjectRequest,
        execution: DelegateExecution
    ): ZaakObjectRequest {
